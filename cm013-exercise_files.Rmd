...
#tick marks that are out of range will not be shown by ggplot (so no problem if i add them)
p_scales +
    scale_x_log10() +
    scale_colour_continuous(
        trans  = "log10", 
...
#doesn't show 90 and 100 because no life Exp. in that range (even though I told it to)
```
...
#ggplot integrates RColorBrewer
#scale_color_distiller
#scale_color_brewer
#one for continuous one for discrete
...
#go to scale_color_  and a bunch of different versions come up
#scale_fill_
```
...
#theme_   and then you can choose the theme of a plot
...
#type axis.  and different options comme up
#function we use: element_text()   > in bracets we can choose anthing such as size, font color etc.
#can use any aspect of the plot
#this doesn't change the grammar just aspects of the plot

#look at theme documentation to find all options
```
...
2. You can save a plotly graph locally as an html file. Try saving the above:
    ...
    save plot doesn't work with ggplotly
