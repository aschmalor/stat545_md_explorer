...
unclass(my_lm) #shows underlying list structure that this object really is
#this list is hard to view and work with so use names function instead (but once we use broom no longer necessary)
names(my_lm)
my_lm$coefficients #to just extract coefficients
```
...
predict(my_lm) %>% 
  head()  
# default: uses data used to fit model to predict data
#uses log of gdp per cap to predict life Exp
# but what we really want is to predict for new data
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
    geom_point() +
    geom_smooth(method="lm") +    #that will fit a model, ggplot just draws regresion line but won't give you any coefficients or info about model fit
    scale_x_log10()
```
...
1. `tidy`: extract statistical summaries about each component of the model.
    ...
    e.g., for linear regression gives you slope and intercept plus certainty around it
...
tidy(my_lm)
augment(my_lm)
augment(my_lm, newdata = tibble(gdpPercap = c(500, 600, 700))) # there are no true y values, they are estimated
glance(my_lm)
#Broom puts zero as p-value if it's close enough t zero
```
